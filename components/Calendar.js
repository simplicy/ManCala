...
export default function BigCalendar({ accountUserList, calendars }) {
  ...
  if (calendars.length && events != null &&
    ...
    //Check for empty calendars, disable them if so
    calendars.map(cal => {
      if (cal.items.length == 0) 
        disabled.push(cal.summary)
    })
    //Add only enabled users to selection list
    accountUserList.users.map(data => {
      selected.push(data)
      if (eventColors.length < accountUserList.users.length)
        eventColors.push({ email: data.email, color: '#' + Math.floor(Math.random() * 16777215).toString(16) })
      if(!isDisabled(data.email))
        account.push(data)
    })
  ...
  const reloadEvents = async () => {
    const userEmails = account.map(data => {
      return data.email
    }).join(",")
      ...
      <Grid container spacing={1}>
        <Grid item xs={2}>
          ...
          <div overflow="hidden" style={{ maxHeight: "25rem", overflowY: "auto" }}>
            {accountUserList.users?.map((userRow, index) => {
